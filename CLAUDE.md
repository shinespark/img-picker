...
- 変更の反映: `content.js` / `popup.*` はページ再読み込みで済むが、`background.js` と `manifest.json` を触ったら拡張の再読み込み（更新ボタン）が必要
...
- **background.js**（service worker）: コンテキストメニュー登録、画像の fetch、キャッシュ書き込み。
...
- **content.js**: 右クリック対象の特定 → PNG 化 → クリップボード書き込み → トースト表示。
...
- **popup.js / popup.html**: キャッシュ一覧の表示、再コピー、削除、保持件数の変更。ポップアップはユーザー操作直後かつフォーカスがあるため、フォールバック無しで `navigator.clipboard.write()` してよい。
